...
// 创建连接池
func newPool(addr, password string, poolSize, maxIdle, idleTimeout, indexDb int) (*redis.Client, error) {
...
func GetPool(addr, password string, poolSize, maxIdle, idleTimeout, indexDb int) (*redis.Client, error) {
	if redisClientPool == nil {
		return newPool(addr, password, poolSize, maxIdle, idleTimeout, indexDb)
	}

	return redisClientPool, nil
}
